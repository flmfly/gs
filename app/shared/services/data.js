...
angular.module('shared.services.data', ['ngResource', 'restangular'])
    .constant('URL_CONFIG', {
        getRootUrl: function () {
            // return 'http://192.168.3.242:8080/apple-invoice-war/';
            return 'http://192.168.1.78:8686/';
            // return window.document.location.pathname.replace('/print/print.html', '/');
        },
        getOrigin: function () {
    ...
    .factory('DataService', ['Restangular', 'URL_CONFIG', '$cacheFactory',
        function (Restangular, URL_CONFIG, $cacheFactory) {
            ...
            return {
                ...
                getSendMessage: function (domainName, data) {
                    return Restangular.service(domainName).one('getmsg').customPOST(data);
                },
                sendMessage: function (domainName, data) {
                    return Restangular.service(domainName).one('msg').customPOST(data);
                },
                ...
                register: function (user) {
                    return this.getBaseService().one('register').post('', user, {}, {});
                }
            };
        }]);