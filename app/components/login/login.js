...
angular.module('components.login', ['shared.services.data', 'shared.services.dialog', 'shared.services.loading', 'shared.session'])

    .controller('LoginController', ['$scope', 'Session', 'DataService', 'DialogService', function ($scope, Session, DataService, DialogService) {
//        $scope.verifyCodeUrl = DataService.getVerifyCodeUrl() + '?_requestTime=' + new Date().getTime();

        $scope.user = {account: '', password: '', newPassword: '', password1: '', needVerify: false};
...
//            	errors.push('新密码必须含有大小写字母和数字！');
        ...
        $scope.$watch('user', function(newVal){
            if(newVal && newVal.needVerify && newVal.needVerify === true){
                $('#verify_code').show();
            }
        });
        ...
        $scope.forgotPassword = function(){
            DialogService.alert(['请联系系统管理员!','电话:010-59112888']);
        };
